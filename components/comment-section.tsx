  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-medium flex items-center">
        ...
        댓글 {comments.reduce((acc, comment) => acc + 1 + comment.replies.length, 0)}
      </h2>

      <div className="flex gap-2">
        <Avatar className="h-8 w-8">
          <AvatarImage src="/placeholder.svg?height=32&width=32" alt="사용자 프로필" />
        ...
        <div className="flex-1">
          ...
          />
          <div className="flex justify-end">
            <Button onClick={handleAddComment} size="sm" disabled={submitting || !newComment.trim()}>
      ...
      <div className="space-y-4 mt-6">
        {comments.length === 0 ? (
          <p className="text-center text-muted-foreground py-8">아직 댓글이 없습니다. 첫 댓글을 작성해보세요!</p>
        ) : (
          comments.map((comment) => (
            <div key={comment.id} className="border rounded-lg p-3">
              <div className="flex gap-2">
                <Avatar className="h-8 w-8">
                  <AvatarImage src={comment.user.avatar || "/placeholder.svg"} alt={`${comment.user.name} 프로필`} />
              ...
              {replyingTo === comment.id && (
                <div className="flex gap-2 mt-2 pl-10">
                  <Avatar className="h-6 w-6">
                    <AvatarImage src="/placeholder.svg?height=24&width=24" alt="사용자 프로필" />
                    <AvatarFallback>사용자</AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <Textarea
                    ...
                    <div className="flex justify-end gap-2">
                      <Button variant="outline" size="sm" onClick={() => setReplyingTo(null)} disabled={submitting}>
                        취소
                      </Button>
                      <Button
                        size="sm"
                ...
                <div className="pl-10 mt-2 space-y-3">
                  {comment.replies.map((reply) => (
                    <div key={reply.id} className="flex gap-2">
                      <Avatar className="h-6 w-6">
                        <AvatarImage src={reply.user.avatar || "/placeholder.svg"} alt={`${reply.user.name} 프로필`} />
